...
instance Show Statement where
  ...
  show (AreNo as bs) = "All no " ++ show as ++ show bs ++ "?"
...
subsetRel kb =
  rtc
    (domain kb)
    ([(x, y) | (x, y, True) <- kb] ++ [(opp y, opp x) | (x, y, True) <- kb])
...
derive kb (AreNo as bs) = opp bs `elem` supersets as kb
derive kb (AreAny as bs) = opp bs `elem` nsupersets as kb
...
report c = map f2s . filter (isRelevant className)
  where
    className = getName c
    isRelevant name (a, b, _) = name == getName a || name == getName b

getName :: Class -> String
getName (Class name) = name
getName (OppClass name) = name
...
tellAbout kb as =
  [All as (Class bs) | (Class bs) <- supersets as kb, as /= Class bs]
    ...
    ++ [Some as (Class bs) | (OppClass bs) <- nsupersets as kb, as /= OppClass bs, (as, Class bs) `notElem` subsetRel kb]
    ++ [SomeNot as (Class bs) | (Class bs) <- nsupersets as kb, as /= Class bs, (as, OppClass bs) `notElem` subsetRel kb]
...
process :: String -> KB
process txt = fromMaybe [] (mapM parse (lines txt) >>= makeKB)
        ...
        Just (What as) ->
          let info = (tellAbout kb as, tellAbout kb (opp as))
           in case info of
                ...
                (posi, _) -> putStrLn (unlines (map show posi))
        Just stmt ->
          if isQuery stmt
            then
              if derive kb stmt
